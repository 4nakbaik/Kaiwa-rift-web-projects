...
import { ChevronLeft, Scroll, Check, X, Lock, Feather } from 'lucide-react';
...
const LockedScreen = ({ current, required }: { current: number, required: number }) => (
    <div className="relative z-10 w-full max-w-lg bg-[#e6e2d3] p-12 border-8 border-[#3e2b22] text-center shadow-[0_20px_50px_rgba(0,0,0,0.9)] font-serif relative overflow-hidden">
        ...
        <div className="mb-6 flex justify-center text-[#5d4037] opacity-80">
        ...
        <div className="w-full bg-[#d7ccc8] h-3 rounded-full mb-8 overflow-hidden border border-[#5d4037] relative z-10">
            <motion.div 
                initial={{ width: 0 }} 
                animate={{ width: `${(current/required)*100}%` }} 
                transition={{ duration: 1 }}
                className="h-full bg-[#8a1c1c]"
            />
        </div>

        <Link to="/flashcards" className="inline-block px-10 py-3 bg-[#1a1a1a] text-[#e6e2d3] font-bold hover:bg-[#333] border-2 border-[#d4af37] uppercase tracking-widest relative z-10 transition-all">
            Kembali Berlatih
        </Link>
    </div>
);
...
const ResultStamp = ({ score, total }: { score: number, total: number }) => {
    ...
    return (
        <motion.div 
            ...
            className={`w-48 h-48 border-[6px] rounded-full flex flex-col items-center justify-center relative mix-blend-multiply mb-8 ${isPassed ? 'border-[#b71c1c] text-[#b71c1c]' : 'border-[#424242] text-[#424242]'}`}
            style={{ maskImage: "url('https://www.transparenttextures.com/patterns/rice-paper.png')" }}
        >
            <div className="absolute inset-0 border-2 rounded-full m-1 opacity-50 border-current"></div>
            <span className="font-calligraphy text-7xl font-black leading-none mt-2">{isPassed ? '合格' : '落第'}</span>
            <span className="font-serif text-xs font-bold uppercase tracking-[0.3em] mt-2 border-t border-current pt-1">{isPassed ? 'PASSED' : 'FAILED'}</span>
            <span className="font-mono text-xl font-bold mt-1 bg-current text-[#e6e2d3] px-2 rounded">{score} / {total}</span>
        </motion.div>
    );
};
...
// --- MAIN EXAM LOGIC ---
const Exam = () => {
    const [gameState, setGameState] = useState<'loading' | 'locked' | 'intro' | 'playing' | 'result'>('loading');
    ...
    if (gameState === 'loading') return (
        <div className="min-h-screen bg-[#0c0c0c] flex items-center justify-center font-sans">
             <SengokuBackground />
             <p className="text-[#e6e2d3] font-calligraphy text-3xl animate-pulse z-10">準備中...</p>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-[#0c0c0c] flex flex-col items-center justify-center relative overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 w-full p-8 flex justify-between items-start z-20">
                <button type="button" title="Kembali ke Intro" onClick={() => setGameState('intro')} className="text-[#e6e2d3] hover:text-[#cd3f3e] transition-colors bg-black/40 p-2 rounded-full backdrop-blur-sm border border-white/10">
                    <ChevronLeft size={24} />
                </button>
            ...
            <div className="relative z-10 w-full max-w-3xl px-6">
                ...
                {/* 2. Question Scroll */}
                <div className="bg-[#e6e2d3] p-12 md:p-16 relative shadow-[0_50px_100px_rgba(0,0,0,1)] border-t-[12px] border-b-[12px] border-[#3e2b22] flex flex-col items-center">
                    {/* Texture */}
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/rice-paper.png')] opacity-30 pointer-events-none"></div>
                    ...
                    <div className="relative z-10 text-center w-full">
                        ...
                        <AnimatePresence mode='wait'>
                            <motion.h1 
                                ...
                                className="text-8xl md:text-9xl font-black text-[#1a1a1a] font-calligraphy mb-12 drop-shadow-sm select-none"
                            >
                                {currentQ.question}
                            </motion.h1>
                        </AnimatePresence>
