...
const Logo = ({ className = "w-10 h-10" }: { className?: string }) => {
    // Jalur SVG untuk membuat garis retakan bergerigi horizontal
    ...
    return (
        <div className={`relative flex items-center justify-center ${className} group select-none`}>
            {/* 1. Background Glow (Lebih halus dari sebelumnya agar tidak mengganggu) */}
            ...
            >
                {/* 2. KANJI: KAI (Dunia/Batas) - Dibuat Putih & Jelas */}
                <span 
                ...
                {/* 3. THE RIFT (Retakan Horizontal di Tengah) */}
                <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none mix-blend-screen">
                    <svg viewBox="0 0 100 100" className="w-[120%] h-full" preserveAspectRatio="none">
                        <defs>
                            {/* Gradient untuk energi retakan */}
                            <linearGradient id="riftEnergy" x1="0%" y1="0%" x2="100%" y2="0%">
                                <stop offset="0%" stopColor="transparent" />
                                <stop offset="20%" stopColor="#22d3ee" /> {/* Cyan */}
                                <stop offset="50%" stopColor="#e879f9" /> {/* Purple/Pink Center */}
                                <stop offset="80%" stopColor="#22d3ee" /> {/* Cyan */}
                                <stop offset="100%" stopColor="transparent" />
                            </linearGradient>
                        </defs>

                        {/* Garis Retakan Utama yang Berpendar */}
                        <motion.path
                            d={jaggedPath}
                            fill="none"
                ...
                {/* 4. Efek Cahaya Horizontal Tipis (Light Leak) tepat di tengah */}
